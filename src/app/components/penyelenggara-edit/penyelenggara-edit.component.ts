...
const GET_ORGANIZER_BYID_API =
  'http://organizer-service-website-lelang-bca-dev.apps.ocpdev.dti.co.id/get/';
const UPDATE_ORGANIZER_API =
  'http://organizer-service-website-lelang-bca-dev.apps.ocpdev.dti.co.id/update/';
...
export class PenyelenggaraEditComponent implements OnInit {
  ...
  getOrganizer(): void {
    ...
    this.http
      .get<any>(GET_ORGANIZER_BYID_API + id, this.httpOptions_base)
      .subscribe(
        (isi) => {
          // console.log(isi)
          // lempar data ke view
  ...
  updateOrganizer(): void {
    if (this.editOrganizer.valid) {
      ...
      this.http
        .put<any>(
          UPDATE_ORGANIZER_API + id,
          this.editOrganizerForm,
